...
# ```
# 1.) I would create a seperate function for this specific issue. For all functions that require the drone to move or change the battery of the drone, I would add this function within mutuator functions. If battery reaches 0 after any movement, I would call upon this function to land the drone.
# 
# 2.) This method, rename_drone() would be a mutator method. The method changes the name of the drone to whatever the user specifies. It changes a property that the object has. This function is similar to the charge(self) method, except that there is another input (self and name).
# 
# 3.) For each mutator method that changes the position of the drone in 3D space, I would reduce the battery by a rate. By multiplying the distance moved by a proper rate, I would reduce the battery by that value. As the distances increase, the battery reduced will be greater. The code would probably look like:
# self.battery -= rate*distance
# This way, the battery's depletion is not a constant amount.
# ```
