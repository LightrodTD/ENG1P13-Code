...
def selection_sort(nums):
    ...
    for i in range(0, n):
        ...
        min_index = i
        #For every element after current index
        for j in range(i, n):
            #Any element smaller than current index will be minimum index
            if new_arr[j] < new_arr[min_index]:
                min_index = j
        #Swap elements in index 'i' and 'min_index'
...
def extract_temps(temps):
    ...
    #Iterate though entire list
    for i in range(len(temps)):
        #If the element can be a float, append to extracted_temps list
        try:
            extracted_temps.append(float(temps[i]))
        #If error occurs, skip element and move on to next one
        except ValueError:
            continue
...
def calculate_median(nums):
    ...
    #If list is even;
    if len(nums)%2 == 0:
        #Add the 2 middles, and find average
        return (nums[int(n-1)] + nums[int(n)])/2
...
def main(temp_data):
    try:
        #Utilize all previous functions to calculate median
        #Extract numeric elements from temp_data, sort list, and calculate median
...
# Boundary Case:
# Function: selection_sort(nums)
# Input: nums = [2.00001,2.00000001]
# Output: [2.00000001, 2.00001]
# Expected Output: [2.00000001, 2.00001]
# Pass/Fail: Pass
# 
# Abnormal Case:
# Function: selection_sort(nums)
...
# ```
# 1.) An input of a list of empty strings would return 'N/A'. If temp_data = [''] (for example), then main(temp_data) will return the string 'N/A', as the function requires. This would be because of the calculate_median() function, which would cause an IndexError. Due to the error, the function will except the error, and return the specified string, 'N/A'.
# 
# 2.) Yes, it is necessary to use try and except statements inside these specific functions. For example, the extract_temps() function returns a list with only floats. If the original list had more than just floats, nor any try and except statements, then the entire function would stop working due to some small insignificant error. In my function at least, the errors are essentially ignored, and the function skips all values that are not floats, or cannot be converted to one. With the input containing multiple data types, error handling is a convenient method that allows the function to run properly.
# ```
