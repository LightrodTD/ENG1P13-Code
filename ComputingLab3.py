...
def extract_grade(x):
    ...
    grade = float(numbers_list[1].strip('%'))
    return grade
...
def class_average(final_marks):
    ...
    for i in range(len(final_marks)):
        answer_list.append(extract_grade(final_marks[i]))
...
def class_std_dev(final_marks):
    answer = 0
    for i in range(len(final_marks)):
        answer += ((extract_grade(final_marks[i]) - class_average(final_marks))**2)    
    std_dev = sqrt(answer/(len(final_marks)))
    return std_dev
...
# ```
# 1.) No, there is no input that would cause the program to fail. As long as the individual elements of the list follow the “studentNum-finalGrade%” format, the program should not fail.
# 
# 2.) The only portion if the code that would require changing is the extract_grade() function. I would need to split the string from ":" instead of "-". I would then need to remove the strip() function, as no "%" is with the number.
# 
# EX:
# def extract_grade(x):
#     numbers_list = x.split(":")
#     grade = float(numbers_list[1])
#     return grade
# 
# 3.) Each function requires a seperate for loop to calculate their specific value. However, each function only requires one loop to solve their specific values.
# 
# ```
